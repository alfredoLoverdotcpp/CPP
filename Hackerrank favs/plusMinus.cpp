...
void plusMinus(vector<int> arr) {
        ...
        for (int i =0; i<n; i++){
             ...
             if (arr[i] == 0){
                 zero_elements.push_back(arr[i]);
             }
             if (arr[i] < 0) {
                 negative_elements.push_back(arr[i]); 
             }
         }
    double positive_ratio =static_cast<double>(positive_elements.size())/n;
    double negative_ratio = static_cast<double>(negative_elements.size())/n; 
    double zero_ratio = static_cast<double>(zero_elements.size())/n;
